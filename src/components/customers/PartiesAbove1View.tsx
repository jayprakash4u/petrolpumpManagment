...
import { Banknote, Search, Copy, Download, FileSpreadsheet, Printer, CheckCircle2, Receipt } from "lucide-react";
import type { PartyAboveThresholdRow } from "@/lib/queries/customers";
import { fmtRs, toNum } from "@/lib/money";
import { Badge } from "@/components/ui/Badge";
import { GhostButton } from "@/components/ui/Button";

const THRESHOLD_NPR = 100000;

/**
 * Customers whose billed total for the selected fiscal year clears
 * NPR 1,00,000 — computed live from the Sale ledger (see
 * `getPartiesAboveThreshold`), not a stored figure that could go stale.
 * The fiscal-year picker re-navigates with `?fy=<startYear>` and the
 * server re-runs the aggregation, the same pattern the Purchase Register
 * uses for its own fiscal-year filter.
 */
export function PartiesAbove1View({
  rows,
  fiscalYears,
  selectedStartYear,
  fiscalYearLabel,
  ...
  const handleDownloadCSV = () => {
    ...
    link.href = url;
    link.download = `parties_above_1_lakh_${fiscalYearLabel.replace("/", "-")}.csv`;
    link.click();
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border/80 pb-3">
        <div>
          ...
          <p className="text-[12px] text-text-muted mt-0.5">
            Billed volume for FY {fiscalYearLabel}, cash and credit combined — for relationship and collection review.
          </p>
        </div>
              ...
              <tbody className="divide-y divide-border/60 text-[11.5px]">
                {filtered.map((p, i) => (
                  <tr key={p.id} className="hover:bg-surface-hi/40 transition-colors whitespace-nowrap text-text">
                    ...
                    <td className="border-r border-border/60 px-3 py-2.5 font-semibold text-text">{p.name}</td>
