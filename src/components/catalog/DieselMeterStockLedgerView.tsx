...
export interface MeterLedgerEntry {
  id: string;
  sn?: number;
  dateBS: string;
  particulars: string;
...
const STORAGE_KEY_DIESEL_METER = "fsm_diesel_meter_stock_ledger_records";
const STORAGE_KEY_PETROL_METER = "fsm_petrol_meter_stock_ledger_records";

const INITIAL_DIESEL_METER_ENTRIES: MeterLedgerEntry[] = [
  {
    id: "dsl-m-open",
    dateBS: "",
  ...
  },
  {
    ...
    sn: 2,
    dateBS: "2083/5/18",
    particulars: "Shrinkage booked 1684",
    inQty: 0,
    outQty: 18.041,
  },
  {
...
const INITIAL_PETROL_METER_ENTRIES: MeterLedgerEntry[] = [
  {
    id: "ptl-m-open",
    dateBS: "",
    particulars: "Opening Stock",
...
interface DieselMeterStockLedgerViewProps {
  fuelName?: string;
  stationName?: string;
  stationAddress?: string;
}

export function DieselMeterStockLedgerView({
  fuelName = "Diesel",
  stationName = "Nepal Petroleum",
  stationAddress = "New Baneshwor-31, Kathmandu",
}: DieselMeterStockLedgerViewProps) {
  const isPetrol = fuelName.toLowerCase().includes("petrol");
  const storageKey = isPetrol ? STORAGE_KEY_PETROL_METER : STORAGE_KEY_DIESEL_METER;
  const initialData = isPetrol ? INITIAL_PETROL_METER_ENTRIES : INITIAL_DIESEL_METER_ENTRIES;

  const [entries, setEntries] = useState<MeterLedgerEntry[]>(() => {
  ...
  const handleDownloadExcel = () => {
    ...
    const csvContent = [
      `"${stationName}"`,
      `"${stationAddress}"`,
      `"${fuelName} Stock Ledger Meter (${activeDateRange.from} to ${activeDateRange.to})"`,
      "",
      headers.join(","),
    ...
    link.href = url;
    link.download = `${fuelName.toLowerCase()}_meter_stock_ledger_${activeDateRange.from}_${activeDateRange.to}.csv`;
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border/80 pb-3 print:hidden">
        <div>
          <h1 className="font-display text-xl font-bold tracking-tight text-text sm:text-2xl flex items-center gap-2">
            ...
            <span>Stock Ledger ({fuelName} Meter Totalizer)</span>
          </h1>
