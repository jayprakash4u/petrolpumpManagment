...
export interface PetrolPurchaseEntry {
  sn: number;
  dateBS: string;
  billNo: string;
...
const MOCK_PETROL_BHADRA_2083: PetrolPurchaseEntry[] = [
  {
    sn: 1,
    dateBS: "2083-Bhadra-1",
    billNo: "30005397",
    density: 736,
    temperature: 27.5,
    tankerNo: "2444NA4KHA",
  ...
  {
    sn: 5,
    dateBS: "2083-Bhadra-5",
    billNo: "30006503",
    density: 733,
    temperature: 31,
    tankerNo: "BP01004KA1002",
...
function generatePetrolMonthData(year: number, monthIdx: number): PetrolPurchaseEntry[] {
  if (year === 2083 && monthIdx === 5) {
    return MOCK_PETROL_BHADRA_2083;
  }
  const tankerList = ["2444NA4KHA", "0974", "BP01004KA1662", "BP01004KA1002", "2281", "4349"];
  const count = 8;
  const rate = 170.50256;
  const rows: PetrolPurchaseEntry[] = [];
  const days = [1, 4, 8, 11, 15, 18, 22, 27];

  for (let i = 0; i < count; i++) {
    const liters = i % 3 === 0 ? 12000 : i % 3 === 1 ? 6000 : 9000;
    ...
    const monthName = MONTHS_NEP.find((m) => m.index === monthIdx)?.name ?? "Month";
    rows.push({
      sn: i + 1,
      dateBS: `${year}-${monthName}-${days[i]}`,
      billNo: String(30005000 + monthIdx * 120 + i * 18),
      density: 730 + (i % 15),
      temperature: 27 + (i % 5),
...
export function PetrolPurchaseReportView({
  initialMonth = 5,
  initialYear = 2083,
  ...
  const [entries, setEntries] = useState<PetrolPurchaseEntry[]>(() =>
    generatePetrolMonthData(initialYear, initialMonth)
  );
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setEntries(generatePetrolMonthData(selectedYear, selectedMonth));
  };
  ...
  const handleExportCSV = () => {
    ...
    link.href = url;
    link.download = `petrol_purchase_report_${appliedYear}_${appliedMonth}.csv`;
      ...
      <div className="rounded-xl border border-border bg-surface p-4 shadow-xs sm:p-6 print:border-none print:bg-white print:p-0 print:shadow-none">
        ...
        <div className="mb-6 text-center">
          ...
          <h3 className="mt-1.5 text-[13px] font-bold text-text">
            {appliedYear} साल {monthObj.name} महिनाको पेट्रोल खरिद विवरण :-
          </h3>
        </div>
              ...
              {entries.length === 0 ? (
                <tr>
                  <td colSpan={14} className="px-3 py-8 text-center text-xs text-text-muted">
                    No petrol purchases recorded for {monthObj.name} {appliedYear}.
                  </td>
                </tr>
              ) : (
                entries.map((row) => (
