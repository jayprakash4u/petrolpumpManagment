...
Write-Host "=================================================="
Write-Host "🚀 Setting up Microsoft SQL Server Database-Per-Tenant"
Write-Host "=================================================="

# 1. Create Master and Tenant Databases
$createDbsSql = @"
IF NOT EXISTS (SELECT * FROM sys.databases WHERE name = 'FuelStationMasterDB')
    CREATE DATABASE [FuelStationMasterDB];

IF NOT EXISTS (SELECT * FROM sys.databases WHERE name = 'FuelStation_shree_petroleum')
    CREATE DATABASE [FuelStation_shree_petroleum];
"@
Execute-Sql "master" $createDbsSql
Write-Host "✅ Databases 'FuelStationMasterDB' and 'FuelStation_shree_petroleum' verified."
...
END;
"@
Execute-Sql "FuelStationMasterDB" $masterDdl
Write-Host "✅ Master Database schema created successfully."

# 3. Tenant DB Schema (for FuelStation_shree_petroleum)
$tenantDdl = @"
...
END;
"@
Execute-Sql "FuelStation_shree_petroleum" $tenantDdl
Write-Host "✅ Tenant Station Database schema created successfully."

Write-Host "=================================================="
Write-Host "🎉 Database-Per-Tenant Provisioning Complete!"
Write-Host "=================================================="
